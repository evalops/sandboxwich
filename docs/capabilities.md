...
| Provider-isolated sidecar (`orb-sidecar`, v1) | Experimental | An apply-mode Kubernetes worker may advertise `provider_isolated_resident_process_v1` only when configured with a digest-pinned `SANDBOXWICH_ISOLATED_RESIDENT_PROCESS_IMAGE` and a nonempty RuntimeClass. The authoritative placed worker creates `orb-sidecar` in a dedicated Pod with its own mount, PID, and network namespaces, an immutable transient bootstrap Secret, no service-account token, a read-only root filesystem, dropped capabilities, non-root identity, seccomp, resource bounds, and a dedicated deny-by-default NetworkPolicy. Its fenced Pod, Secret, and NetworkPolicy names include the generation and lease, and cleanup targets only those exact resources. The policy denies ingress and permits only cluster DNS plus public HTTPS with the configured control-plane/metadata CIDRs excluded. Pending Pods do not acknowledge bootstrap and fail retryably after `SANDBOXWICH_ISOLATED_RESIDENT_PROCESS_STARTUP_TIMEOUT_SECS`; observation uses configurable bounded backoff. This is a real boundary from the guest Pod, but its strength still depends on the operator's RuntimeClass and cluster/kernel isolation. The sidecar does **not** share guest localhost; any communication channel must use an explicit HTTPS endpoint or relay. Only the placed worker can claim or observe sidecar work, while `orb-executor` remains guest-owned. Bootstrap delivery is generation/lease/digest-fenced and retryable until container-start evidence permits the exact `Starting` acknowledgment, subject to the same process-local limitation above. Once a sidecar row exists, executor bootstrap fails closed unless the sidecar is `Running` under a live lease. Bounded-label events and metrics expose bootstrap-block reasons and terminal resident failures without secret material or tenant identifiers. |

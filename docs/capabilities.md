...
| Sidecar placement primitive (`orb-sidecar`, v1) | Experimental, disclosed weaker than a trust boundary | evalops/orb#296's child issue #176. A tenant may additionally create one `orb-sidecar` resident process per sandbox (same one-per-sandbox, generation-fenced, one-read-bootstrap contract as `orb-executor`, reusing that mechanism to deliver the sidecar's own claim credential). **This is NOT a separate trust domain.** The sidecar runs as an ordinary resident process *inside* the same sandbox/container as the agent workload, merely under a different fixed uid (`10111`) than the default guest container uid (`10001`) that `orb-executor` and every other resident process inherit. A root -- or otherwise sufficiently privileged -- agent process in that same sandbox can still read the sidecar's files, `ptrace` it, or otherwise recover its secret material; uid separation only stops accidental/unprivileged cross-process access, not a hostile privileged one. The worker-level primitive that places the sidecar entirely outside the sandbox's mount/net namespaces (the actual separate-trust-domain target) is tracked as separate future work, not implemented here. **Privilege-drop is fail-closed, not fail-open:** the guest agent must advertise uid-isolated-resident-process v1 support before the API creates or leases `orb-sidecar`; the agent then attempts a real `setuid`/`setgid` and transfers the private bootstrap file to uid/gid `10111` before spawn. If it lacks that privilege (true for the default Kubernetes apply-mode pod today, which is not granted `SETUID`/`SETGID`), the spawn fails outright and the resident process observes `Failed` -- it never silently runs the sidecar under the workload's own uid. Operators who want the sidecar to actually start must arrange for the agent process to hold `SETUID`/`SETGID` narrowly (e.g. a dedicated, file-capability-scoped launcher) -- the Kubernetes provider intentionally does *not* grant this capability at the container level, because doing so would also hand it to the agent workload itself and let a compromised workload `setuid()` into the sidecar's identity directly, defeating the separation it is meant to provide. **Fail-closed dependent operation:** once a sandbox has ever had an `orb-sidecar` configured (a row exists, even if later stopped), `orb-executor`'s one-read bootstrap-credential request is refused with `503 resident_sidecar_unavailable` unless the sidecar is currently observed `Running` and that observation is backed by a live, unexpired lease. This is the one operation v1 gates -- other resident-process actions (spawn, observations, stop) for either name are unaffected, and sandboxes that never configure a sidecar see no behavior change. |

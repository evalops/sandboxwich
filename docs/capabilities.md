...
| Foam compiler-cache restore | Experimental | Standard runtime Pods use a root init container and root restore helper sharing only `/workspace`. The UID-10001 workload cannot select the helper through ordinary exec. Archives stage beneath a root-only directory, activate with a no-clobber same-filesystem rename into a root-owned parent, and become workload-owned only after a bottom-up ownership handoff. This is an intra-Pod filesystem-integrity boundary, not isolation from a root workload or a compromised Kubernetes control plane. |

...
| Managed persistent homes | Experimental | A durable home admits at most one live sandbox. `planning` through `archiving` retain the mount; `archived` and `error` are replacement-eligible through lazy release. Callers adopt or wait after typed `409 home_already_mounted`; see the [persistent home lifecycle contract](persistent-home-lifecycle.md). |
...
| MCP box facade (`POST /mcp` and `POST /v1/mcp`) | Experimental | Tenant-authenticated Streamable-HTTP JSON-RPC. Tools: `box_create`, `box_list`, `box_get`, `box_exec`, `box_snapshot`, `box_fork`, `box_sleep`, `box_wake`, `box_destroy`, `box_sizes`. `box_create` forces `workspace_mode=persistent` and refuses an uncapped lifetime unless the operator set `SANDBOXWICH_DEFAULT_MAX_LIFETIME_SECONDS` or `SANDBOXWICH_DEFAULT_IDLE_TTL_SECONDS`. `box_destroy` requires `confirm=true` and archives via stop. `box_sleep` snapshots then stops; `box_wake` resumes disk state only. No GPU catalog, no public SSH tunnel, no Orb sleep/wake. Exercised by `http_contract::mcp`. |

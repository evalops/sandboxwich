...
fn agent_sandbox_signing_key_loader_accepts_pkcs8_v1_without_public_key() {
    ...
    let file = tempfile::NamedTempFile::new().expect("temporary signing key");
    std::fs::write(
        file.path(),
        base64::engine::general_purpose::STANDARD.encode(key_bytes),
    )
    .expect("write encoded signing key");

    assert!(load_agent_sandbox_signing_key(file.path().to_str().unwrap()).is_ok());
}

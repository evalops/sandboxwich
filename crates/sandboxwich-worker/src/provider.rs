...
impl KubernetesApplyProvider {
        ...
        F: FnMut(ProvisioningStageUpdateRequest) -> anyhow::Result<()>,
    {
        ...
        //
        // These three checks stay outside the rollback guard below on purpose:
        // a provider that fails `validate_apply_gate` has mutation disabled, so
        // it must not issue the rollback `kubectl delete` either.
        ...
        // (the control plane) rather than out of kubectl. Rolling back on a
        // control-plane rejection is not safe: the usual reason a stage update
        // is rejected is that this attempt no longer owns the sandbox's lease,
        // and a label-scoped delete would then tear down the resources the
        // *new* owner has already applied under the same sandbox id. Residue
        // from that case is instead reaped by the unbound-workspace-claim
        // backstop in `classify_reconciliation`, which is exactly the shape it
        // recognizes: an applied claim the control plane holds no record of.
        ...
        if result.is_err() && !report_rejected.get() {
            ...
            // a readiness timeout -- used to leave those earlier objects behind
            // with nothing owning them. On 2026-08-03 that leaked 2,772 Pending
            // `sandboxwich-pvc-*` claims into evalops-sandboxes in three hours,
            // because quota-rejected provisions retried every few seconds and
            // each attempt staged one more claim. The unstaged `provision` path
            // has rolled back this way since the original leak fix; this makes
            // the staged path match it.
            //
            // Label-scoped, so it can only ever remove objects carrying this
            // sandbox's id: a managed home claim (`sandboxwich-home-*`, labelled
            // with a home id and no sandbox id) is not matched, and neither is
            // any other sandbox's workspace.
            self.rollback_applied_resources(sandbox_id, "provision (staged)");
        }
        result
    }

    /// Applies the staged manifest sequence. Split out of
    /// [`Self::provision_staged_with_home`] so every failure between the first
    /// apply and `SandboxReady` runs through one rollback, rather than needing a
    /// cleanup call bolted onto each `?`.
    fn provision_staged_resources<F>(
        &self,
        sandbox_id: SandboxId,
        home_id: Option<HomeId>,
        spec: &SandboxProvisionSpec,

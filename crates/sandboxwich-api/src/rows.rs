...
pub(crate) fn row_to_resident_process(row: AnyRow) -> Result<ResidentProcess, ApiError> {
    ...
    let env: &str = row.try_get("env")?;
    let restart_policy: &str = row.try_get("restart_policy")?;
    let desired_state: &str = row.try_get("desired_state")?;
    let observed_state: &str = row.try_get("observed_state")?;
    let generation: i64 = row.try_get("generation")?;
    let active_lease_id: Option<&str> = row.try_get("active_lease_id")?;
    ...
    Ok(ResidentProcess {
        ...
        active_lease_id: active_lease_id.map(parse_uuid).transpose()?,
        pid: pid
            .map(u32::try_from)
            .transpose()
        ...
        exited_at: exited_at.map(parse_timestamp).transpose()?,
        exit_code: exit_code
            .map(i32::try_from)
            .transpose()
...
pub(crate) fn row_to_stored_sandbox_file(row: AnyRow) -> Result<StoredSandboxFile, ApiError> {
    let content_base64: &str = row.try_get("content_base64")?;
...
pub(crate) fn row_to_runtime_resource(row: AnyRow) -> Result<RuntimeResource, ApiError> {
    let id: &str = row.try_get("id")?;
    let sandbox_id: &str = row.try_get("sandbox_id")?;
    let snapshot_id: Option<&str> = row.try_get("snapshot_id")?;
    let resource_kind: &str = row.try_get("resource_kind")?;
    let purpose: &str = row.try_get("purpose")?;
    let status: &str = row.try_get("status")?;
    let service_port: Option<i64> = row.try_get("service_port")?;
    let source_snapshot_id: Option<&str> = row.try_get("source_snapshot_id")?;
    let created_at: &str = row.try_get("created_at")?;
    let updated_at: &str = row.try_get("updated_at")?;
    let observed_at: Option<&str> = row.try_get("observed_at")?;
    let last_reconciled_at: Option<&str> = row.try_get("last_reconciled_at")?;
    let ready_at: Option<&str> = row.try_get("ready_at")?;
    let deleted_at: Option<&str> = row.try_get("deleted_at")?;
    ...
    Ok(RuntimeResource {
        ...
        created_at: parse_timestamp(created_at)?,
        updated_at: parse_timestamp(updated_at)?,
        observed_at: observed_at.map(parse_timestamp).transpose()?,
        last_reconciled_at: last_reconciled_at.map(parse_timestamp).transpose()?,
        ready_at: ready_at.map(parse_timestamp).transpose()?,
...
pub(crate) fn row_to_desktop_session(row: AnyRow) -> Result<DesktopSession, ApiError> {
    ...
    let sandbox_id: &str = row.try_get("sandbox_id")?;
    let status: &str = row.try_get("status")?;
    let access_mode: &str = row.try_get("access_mode")?;
    let connection_metadata: &str = row.try_get("connection_metadata")?;
    let created_at: &str = row.try_get("created_at")?;
    let updated_at: &str = row.try_get("updated_at")?;
    let expires_at: Option<&str> = row.try_get("expires_at")?;
...
pub(crate) fn row_to_worker(row: AnyRow) -> Result<Worker, ApiError> {
    let id: &str = row.try_get("id")?;
    let status: &str = row.try_get("status")?;
    let capabilities: &str = row.try_get("capabilities")?;
    let max_concurrent_jobs: i64 = row.try_get("max_concurrent_jobs")?;
    let labels: &str = row.try_get("labels")?;
    // Optional until every SELECT lists the column; missing/empty means no
    // observed capacity evidence has been heartbeated yet.
    let resource_envelope: Option<&str> = row.try_get("resource_envelope").unwrap_or(None);
    let registered_at: &str = row.try_get("registered_at")?;
    let last_heartbeat_at: Option<&str> = row.try_get("last_heartbeat_at")?;
    ...
    Ok(Worker {
        ...
        last_heartbeat_at: last_heartbeat_at.map(parse_timestamp).transpose()?,
    })
}
...
pub(crate) fn row_to_guest_health(row: AnyRow) -> Result<GuestHealth, ApiError> {
    let sandbox_id: &str = row.try_get("sandbox_id")?;
    let status: &str = row.try_get("status")?;
    let last_probe_at: &str = row.try_get("last_probe_at")?;
    let checks: &str = row.try_get("checks")?;
...
pub(crate) fn row_to_ssh_key(row: AnyRow) -> Result<SshKey, ApiError> {
    ...
    let sandbox_id: &str = row.try_get("sandbox_id")?;
    let status: &str = row.try_get("status")?;
    let requested_at: &str = row.try_get("requested_at")?;
    let updated_at: &str = row.try_get("updated_at")?;
    let applied_at: Option<&str> = row.try_get("applied_at")?;
    ...
    Ok(SshKey {
        ...
        updated_at: parse_timestamp(updated_at)?,
        applied_at: applied_at.map(parse_timestamp).transpose()?,
        error: row.try_get("error")?,
    })
}

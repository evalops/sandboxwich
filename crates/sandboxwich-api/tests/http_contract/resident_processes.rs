...
#[tokio::test]
async fn unprivileged_sandbox_cannot_start_maestro_hosted_runner() {
    let data_dir = tempfile::tempdir().unwrap();
    let server = TestServer::start(
        format!(
            "sqlite://{}",
            data_dir.path().join("maestro-profile.db").display()
        ),
        Some(data_dir),
    )
    .await;
    let (sandbox_id, _, _) =
        provisioned_sandbox_with_guest(&server, "maestro-hosted-service", false).await;
    let request = ResidentProcessRequest {
        argv: vec![
            MAESTRO_HOSTED_RUNNER_BINARY.into(),
            "hosted-runner".into(),
            "--listen".into(),
            "0.0.0.0:8080".into(),
        ],
        ...
        env: BTreeMap::from([
            (
                MAESTRO_HOSTED_RUNNER_AUTH_TOKEN_FILE_ENV.into(),
                MAESTRO_HOSTED_RUNNER_AUTH_TOKEN_FILE.into(),
            ),
            (
                "MAESTRO_RUNNER_SESSION_ID".into(),
                "runner-session-1".into(),
            ),
            (
                "MAESTRO_REMOTE_RUNNER_OWNER_INSTANCE_ID".into(),
                "owner-generation-1".into(),
            ),
            ("MAESTRO_WORKSPACE_ROOT".into(), "/workspace".into()),
            (
                "MAESTRO_HEADLESS_CLI_PATH".into(),
                MAESTRO_HOSTED_RUNNER_BINARY.into(),
            ),
        ]),
        restart_policy: ResidentProcessRestartPolicy::OnFailure,
        expected_generation: 0,
        bootstrap: Some(ResidentProcessBootstrap {
            content: b"unprivileged-test-token".to_vec(),
            target_file: MAESTRO_HOSTED_RUNNER_AUTH_TOKEN_FILE.into(),
            mode: 0o600,
        }),
    };
    ...
    assert!(error.message.contains("maestro_hosted_runner_v1"));
}

#[tokio::test]
async fn maestro_hosted_runner_profile_accepts_the_typed_resident_generation() {
    let data_dir = tempfile::tempdir().unwrap();
    let server = TestServer::start(
        format!(
            "sqlite://{}",
            data_dir.path().join("maestro-resident.db").display()
        ),
        Some(data_dir),
    )
    .await;
    let (sandbox_id, _, _) =
        provisioned_sandbox_with_guest(&server, "maestro-resident", false).await;
    sqlx::any::install_default_drivers();
    let pool = AnyPool::connect(&server.database_url).await.unwrap();
    sqlx::query("update sandboxes set runtime_profile = ? where id = ?")
        .bind(SandboxRuntimeProfile::MaestroHostedRunnerV1.as_db_str())
        .bind(sandbox_id.to_string())
        .execute(&pool)
        .await
        .unwrap();

    let plaintext_auth = server
        .client()
        .put(format!(
            "{}/sandboxes/{sandbox_id}/resident-processes/{MAESTRO_HOSTED_RUNNER_RESIDENT_PROCESS_NAME}",
            server.base_url
        ...
        .json(&ResidentProcessRequest {
            ...
            cwd: Some("/workspace".into()),
            env: BTreeMap::from([(
                "MAESTRO_HOSTED_RUNNER_AUTH_TOKEN".into(),
                "must-not-cross-the-api".into(),
            )]),
            restart_policy: ResidentProcessRestartPolicy::OnFailure,
            expected_generation: 0,
            bootstrap: None,
        ...
        .await
        .unwrap();
    assert_eq!(plaintext_auth.status(), reqwest::StatusCode::BAD_REQUEST);

    let valid_request = ResidentProcessRequest {
        argv: vec![
            MAESTRO_HOSTED_RUNNER_BINARY.into(),
            "hosted-runner".into(),
            "--listen".into(),
        ...
        expected_generation: 0,
        bootstrap: Some(ResidentProcessBootstrap {
            content: b"hosted-runner-test-token".to_vec(),
            target_file: MAESTRO_HOSTED_RUNNER_AUTH_TOKEN_FILE.into(),
            mode: 0o600,
        }),
    };
    let mut repeated_listener = valid_request.clone();
    repeated_listener
        .argv
        .extend(["--listen".into(), "127.0.0.1:9090".into()]);
    let repeated_listener = server
        .client()
        .put(format!(
            "{}/sandboxes/{sandbox_id}/resident-processes/{MAESTRO_HOSTED_RUNNER_RESIDENT_PROCESS_NAME}",
            server.base_url
        ))
        .json(&repeated_listener)
        .send()
        .await
        .unwrap();
    assert_eq!(repeated_listener.status(), reqwest::StatusCode::BAD_REQUEST);

    let response: ResidentProcessResponse = server
        .client()
        .put(format!(
            "{}/sandboxes/{sandbox_id}/resident-processes/{MAESTRO_HOSTED_RUNNER_RESIDENT_PROCESS_NAME}",
            server.base_url
    ...
    assert_eq!(
        response.resident_process.name,
        MAESTRO_HOSTED_RUNNER_RESIDENT_PROCESS_NAME
    );
    assert_eq!(response.resident_process.generation, 1);
    assert_eq!(
        response.resident_process.observed_state,

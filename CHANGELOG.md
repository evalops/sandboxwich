...
- Sandbox creation now carries a durable typed `executionClass`. Callers choose
  the provider-neutral workload class, while operators configure the worker
  isolation profile, RuntimeClass, compatible nodes, CNI, storage, and live
  conformance. VM-class execution remains experimental pending SW-3 live
  certification.
